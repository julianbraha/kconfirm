...
#[test]
fn test_constant_condition_kconfig_fixtures() {
    let constant_condition_dir = std::path::Path::new("tests/fixtures/constant_condition");

    let entries = fs::read_dir(constant_condition_dir).expect("failed to read dead code directory");
